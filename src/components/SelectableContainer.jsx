...
const SelectableContainer = (props) => {
  ...
  useEffect(() => {
      ...
      return () => {
          ...
          selectableRef.current.removeEventListener(
              'mousedown',
              handleMouseDown
          );
          selectableRef.current.removeEventListener(
  ...
  const updateRegistry = () => {
      ...
      for (const segment of segmentsRef.current) {
          segment.current.hBounds = getBoundsForNode(
              segment.current,
              scrollInfo
          );
      }
  ...
  const checkScrollTop = (clientY, currentTop) => {
      ...
      if (offset > 0 || clientY < 0) {
          scrollContainerRef.current.scrollTop =
              currentTop - getScrollStep(offset);
      }
  };
  ...
  const handleMouseMove = (e) => {
      ...
      const pointY =
          ...
          0 + containerScrollRef.current.scrollTop;
      const selectboxY = Math.min(
          pointY,
          mouseDownDataRef.current.selectboxY
      );

      const pointX =
          clientX - scrollBoundsRef.current?.left ??
          0 + containerScrollRef.current.scrollLeft;
      const selectboxX = Math.min(
          pointX,
          mouseDownDataRef.current.selectboxX
      );
      ...
      selectBoxRef.current.style.width =
          selectboxStateRef.current.width + 'px';
  ...
  const handleMouseUp = (e) => {
      ...
      if (
          ...
          handleClick(evt, clientY, clientX);
      } else {
          if (
              evt.which === 1 &&
              mouseDownDataRef.current.target === evt.target
          ) {
          ...
          selectBoxRef.current.style.left =
              selectboxStateRef.current.x + 'px';
          selectBoxRef.current.style.top = selectboxStateRef.current.y + 'px';
          selectBoxRef.current.style.width =
              selectboxStateRef.current.width + 'px';
          selectBoxRef.current.style.height =
